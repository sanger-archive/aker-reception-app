...
RSpec.feature 'Barcode Scanner', type: :feature do
  describe "when visiting the 'Reception' page" do
    let(:user) { OpenStruct.new(email: 'user@sanger.ac.uk', groups: %w[world pirates]) }
    ...
    before do
      ...
      @material_submission = FactoryBot.create(:material_submission,
                                               status: 'printed',
                                               dispatched: true,
                                               dispatch_date: Time.current)
      @material_submission.labwares = [FactoryBot.create(:labware_with_barcode,
    ...
    context 'scanning a barcode', js: true do
      it 'allows scanning of a correctly formatted barcode with no leading or trailing spaces' do
        find('#material_reception_barcode_value')
          .set("#{@material_submission.labwares[0].barcode}\n")
          .native.send_keys(:return)
        expect(page).to have_content(@material_submission.labwares[0].barcode)
      end
