...
export default (state = {}, action) => {
  ...
  switch(action.type) {
    case C.MATCH_SELECTION:
      let mp = Object.assign({}, state.mapping_tool)
      ...
      return Object.assign({}, state, {mapping_tool: mp})
    case C.UNMATCH:
      let mapping_tool = Object.assign({}, state.mapping_tool)
      mapping_tool.matched = mapping_tool.matched.filter((elem) => {
        return !((elem.observed == action.observed) && (elem.expected == action.expected))
      })
      mapping_tool.observed.push(action.observed)
      mapping_tool.expected.push(action.expected)
      return Object.assign({}, state, {mapping_tool})
