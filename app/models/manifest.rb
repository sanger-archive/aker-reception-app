class Manifest < ApplicationRecord
  ...
  scope :active, -> { where(status: Manifest.ACTIVE) }
  scope :printed, -> { where(status: Manifest.PRINTED) }
  # broken manifests are not listed
  scope :pending, -> { where(status: [nil, 'labware', 'provenance', 'ethics', 'dispatch']) }
  scope :for_user, ->(owner) { where(owner_email: owner.email) }

  def create_uuid
    self.manifest_uuid ||= SecureRandom.uuid
  end
  ...
  def active_or_printed?
    return false if status.nil?
    active? || status == Manifest.PRINTED
  end
  ...
  def broken!
    update_attributes(status: Manifest.BROKEN)
  end
  ...
  # Get the total number of samples for this manifest
  ...
  # Deletes any labware linked to this manifest, and creates
