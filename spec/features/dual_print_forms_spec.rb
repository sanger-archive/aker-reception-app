...
RSpec.feature "DualPrintForms", type: :feature do
  ...
  describe "printer selection", js: true do
    ...
    let!(:printers) { printer_names.map { |name| create(:printer, name: name) } }
    ...
    context "when the page is loaded" do
      it "shows both dropdown menus with all available printers" do
        expect(page).to have_select("printer_name_top", options: printer_names)
        expect(page).to have_select("printer_name_bottom", options: printer_names)
      end
    ...
    context "when choosing from the top dropdown" do
      it "mirrors the option to the bottom dropdown" do
        expect(page).to have_select("printer_name_top", selected: printer_names[0])
        select printer_names[1], from: "printer_name_top"
        wait_for_ajax
        expect(page).to have_select("printer_name_bottom", selected: printer_names[1])
      end
    end

    context "when choosing from the bottom dropdown" do
      it "mirrors the option to the top dropdown" do
        expect(page).to have_select("printer_name_bottom", selected: printer_names[0])
        select printer_names[1], from: "printer_name_bottom"
        wait_for_ajax
        expect(page).to have_select("printer_name_top", selected: printer_names[1])
      end
    end
