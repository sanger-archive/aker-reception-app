...
RSpec.feature "DualPrintForms", type: :feature do
  ...
  describe "printer selection", js: true do
    ...
    let(:options) { printers.map(&:name_and_type) }
    ...
    context "when the page is loaded" do
      it "shows both dropdown menus with all available printers" do
        expect(page).to have_select("printer_name_top", options: options)
        expect(page).to have_select("printer_name_bottom", options: options)
      end
    ...
    context "when choosing from the top dropdown" do
      it "mirrors the option to the bottom dropdown" do
        expect(page).to have_select("printer_name_top", selected: options[0])
        select options[1], from: "printer_name_top"
        wait_for_ajax
        expect(page).to have_select("printer_name_bottom", selected: options[1])
      end
    end

    context "when choosing from the bottom dropdown" do
      it "mirrors the option to the top dropdown" do
        expect(page).to have_select("printer_name_bottom", selected: options[0])
        select options[1], from: "printer_name_bottom"
        wait_for_ajax
        expect(page).to have_select("printer_name_top", selected: options[1])
      end
    end
